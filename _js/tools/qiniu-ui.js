function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        //img
        var progressImg = $("<td/>");
        progressImg.addClass('progressImg');

        //info
        var fileSize = plupload.formatSize(file.size).toUpperCase();
        var progressText = $("<td/>");
        progressText.addClass('progressName').html(
            '<div><strong>name:</strong><span class="origin-format">' + file.name + '</span></div>' +
            '<div><strong>size:</strong><span class="origin-format">' + fileSize + '</span></div>'
        );
        //
...
FileProgress.prototype.setComplete = function (up, info) {
    ...
    if (res.url) {
        url = res.url;
        str =
            "<div><strong>Link:</strong><a href=" + res.url + " target='_blank' > " + res.url + "</a></div>" +
            "<div><strong>Md:</strong><span>![](" + res.url + ")</span></div>" +
            "<div><strong>CSS:</strong><span>background-image: url(" + res.url + ");</span></div>";
    } else {
        var domain = up.getOption('domain');
        url = domain + encodeURI(res.key);
        var link = domain + res.key;
        str =
            "<div><strong>Link:</strong><a href=" + url + " target='_blank' > " + link + "</a></div>" +
            "<div><strong>Md:</strong><span>![](" + url + ")</span></div>" +
            "<div><strong>CSS:</strong><span class='css-value'>" +
    ...
    var progressImgTd = this.fileProgressWrapper.find('.progressImg');
    var imageView = '?imageView2/2/w/200';
    ...
    var showImg = $('<img class="loading" src="https://o4j4l4n7h.qnssl.com/21286c8f87655564753f7d602c31385bea7fd85f2fcf1fd06-rMeG10_fw658.gif"/>');
    progressImgTd.append(linkWrapper);

    if (!isImg) {
        showImg.attr('src', 'https://o4j4l4n7h.qnssl.com/default.png');
        ...
        $(img).on('load', function () {
            ...
            linkWrapper.attr('href', orginUrl).attr('title', '查看原图');
            ...
            function initImg(url, key, height) {
                ...
                modalBody.find('img').attr('src', 'https://o4j4l4n7h.qnssl.com/21286c8f87655564753f7d602c31385bea7fd85f2fcf1fd06-rMeG10_fw658.gif');
            ...
            var infoWrapper = $('<div class="infoWrapper"></div>');
            var ie = Qiniu.detectIEVersion();
            if (!(ie && ie <= 9)) {
                var imageInfo = Qiniu.imageInfo(res.key);

                var infoInner =
                    '<div><strong>type:</strong><span class="origin-format">' + imageInfo.format + '</span></div>' +
                    '<div><strong>width:</strong><span class="orgin-width">' + imageInfo.width + 'px</span></div>' +
                    '<div><strong>height:</strong><span class="origin-height">' + imageInfo.height + 'px</span></div>';

                var css =
                    "width: " + imageInfo.width / 2 + "px;</br>" +
                    "height: " + imageInfo.height / 2 + "px;"
            }

            progressNameTd.append(infoInner);
            var cssValue = td.find('.css-value');
            cssValue.append(css);

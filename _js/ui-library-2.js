...
I18N['zh-cn'] = {
    ...
    "Plugins": "插件",
    "Toolbar": "工具栏",
    "1. Mark Overlay": "1. 标注区域",
    "2. Mark Sizes": "2. 标注尺寸",
    "3. Mark Spacings": "3. 标注间隔",
    "4. Mark Properties": "4. 标注属性",
    "5. Mark Note": "5. 标注备注",
    "Spec Export": "规范导出",
    "Toggle Hidden": "切换隐藏标注",
    "Toggle Locked": "切换锁定标注",
    "Clear Marks": "清除标注",
    "Settings": "设置",
    "Color Names": "颜色命名",
    "Make Exportable": "设置切图",
    ...
    SMApp.fn = SMApp.prototype = {
        ...
        screen: function () {
            ...
            $('#screen').css({
                ...
                background: '#FFF url(' + openId + imageData + ') no-repeat',
        ...
        artboards: function () {
            ...
            $.each(this.project.artboards, function (index, artboard) {
                ...
                artboardListHTML.push(
                    ...
                    '<img alt="' + artboard.name + '" src="' + openId + imageData + '">',
                    '</picture>',
                    '<div>',
                    '<h3>' + artboard.name + '</h3>',
            ...
            $.each(this.project.slices, function (index, sliceLayer) {
                if (sliceLayer.exportable.length > 0) {
                    ...
                    sliceListHTML.push(
                        ...
                        '<picture><img src="' + openId + 'assets/' + asset.path + '?' + timestamp + '" alt=""></picture>',
                        '<div>',
                ...
                if (!this.isIntersect(selectedRect, targetRect)) {
                    ...
                    if (selectedRect.x > targetRect.maxX) { //left
                        leftData = {
                            ...
                            percentageDistance: this.percentageSize((selectedRect.x - targetRect.maxX), this.current.height)
                        }
                    }
                }
                else {
        ...
        inspector: function () {
            ...
            if (layerData.exportable && layerData.exportable.length > 0) {
                ...
                $.each(layerData.exportable, function (index, exportable) {
                    ...
                    expHTML.push(
                        '<li>',
                        '<a href="' + openId + filePath + '" download="'+ exportable.path +'" data-format="' + exportable.format.toUpperCase() + '"><img src="' + openId + filePath + '" alt="' + exportable.path + '">' + exportable.path.replace('drawable-', '') + '</a>',
                        '</li>');
                });
                expHTML.push('</ul>')
